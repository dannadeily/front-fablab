    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const poblacionesWithIsEnabled = response.data.map(
                    (poblacionItem) => ({
                        ...poblacionItem,
                        isEnabled: poblacionItem.status,
                    })
                );
                setName(poblacionesWithIsEnabled);
            } catch (error) {
                console.error(error);
            }
        };
                ...
                setName((prevState) =>
                    prevState.map((poblacionItem) =>
                        poblacionItem.id === id
                            ? {
                                  ...poblacionItem,
                                  isEnabled: !poblacionItem.isEnabled,
                              }
                            : poblacionItem
                    )
                );
    ...
    return (
        <div className="md:w-1/2 lg:w-3/5 md:h-screen overflow-y-scroll">
            {name && name.length ? (
                        ...
                        >
                            ...
                            <div className="flex justify-between ">
                                <button
                                    ...
                                    onClick={() =>
                                        handleToggleEstado(poblacionItem.id)
                                    }
                                >
                ...
                <>
                    <h2 className="font-black text-2xl text-center">
                        No hay tipos de poblaciones registradas
                    </h2>
                    <p className="text-xl mt-5 mb-10 text-center">
